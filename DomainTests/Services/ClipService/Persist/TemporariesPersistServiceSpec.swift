                    ...
                    context("一時保存されたタグを永続化できる") {
                        ...
                        context("永続化成功した一時保存されたタグのDirtyフラグを折ることができる") {
                            beforeEach {
                                ...
                                result = service.persistIfNeeded()
                            }
                            ...
                            it("トランザクションの開始/終了が行われる") {
                                ...
                                // 一時保存領域のクリーン分が+されている
                                expect(temporaryClipStorage.beginTransactionCallCount).to(equal(2))
                                expect(temporaryClipStorage.commitTransactionCallCount).to(equal(2))
                        ...
                        context("永続化成功した一時保存されたタグのDirtyフラグを折るのに失敗した") {
                            ...
                            it("クリップのメタ情報及び画像データの永続化は実施されない") {
                                // TODO:
                            }
