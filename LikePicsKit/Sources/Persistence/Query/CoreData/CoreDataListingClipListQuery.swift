...
class CoreDataListingClipListQuery: NSObject {
    ...
    private var clipIds: Set<Domain.ListingClip.Identity>
    private var subject: CurrentValueSubject<[Domain.ListingClip], Error>
    ...
    init(requestFactory: @escaping RequestFactory, context: NSManagedObjectContext) throws {
        ...
        let currentClips = try context.fetch(request)
            .compactMap { $0.map(to: Domain.ListingClip.self) }
...
extension CoreDataListingClipListQuery: NSFetchedResultsControllerDelegate {
                    ...
                    didChangeContentWith snapshot: NSDiffableDataSourceSnapshotReference)
    {
        controller.managedObjectContext.perform { [weak self] in
            guard let self = self else { return }
            let clips: [Domain.ListingClip] = (snapshot as NSDiffableDataSourceSnapshot<Int, NSManagedObjectID>).itemIdentifiers
...
extension CoreDataListingClipListQuery: ListingClipListQuery {
    // MARK: - ClipListQuery

    var clips: CurrentValueSubject<[Domain.ListingClip], Error> {
        return self.subject
    }
}

extension CoreDataListingClipListQuery: ViewContextObserver {
