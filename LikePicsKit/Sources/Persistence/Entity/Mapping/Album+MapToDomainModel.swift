...
extension Persistence.Album {
    public func map(to: Domain.Album.Type) -> Domain.Album? {
...
extension Persistence.Album {
    public func map(to: Domain.ListingAlbumTitle.Type) -> Domain.ListingAlbumTitle? {
        guard let id = self.id,
            let title = self.title,
            let createdDate = self.createdDate,
            let updateDate = self.updatedDate
        ...
        return Domain.ListingAlbumTitle(
            id: id,
            title: title,
            isHidden: self.isHidden,
            registeredDate: createdDate,
